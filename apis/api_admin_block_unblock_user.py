...
def _():
    try:
        ...
        if user["user_blocked_until"] > int(time.time()):
            # Unblock
            print("Unblock")
            total_rows_updated = db.execute("UPDATE users SET user_blocked_until = 0 WHERE user_id = ?", (user["user_id"],)).rowcount
            ...
            total_rows_updated = db.execute("UPDATE users SET user_blocked_until = ? WHERE user_id = ? AND user_admin = 0", (user_blocked_until, user["user_id"],)).rowcount
