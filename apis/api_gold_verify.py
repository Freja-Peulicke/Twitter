...
def _():
    try:
        ...
        db=x.db()
        user = db.execute("SELECT * FROM users WHERE user_id = ? AND user_gold_key = ? LIMIT 1", (logged_in_user["user_id"], verify_code)).fetchone()
        if not user: raise Exception(400, "Wrong code")
        gold_at = int(time.time())
        total_rows_updated = db.execute("UPDATE users SET user_gold_at = ?, user_gold_key = 0 WHERE user_id = ?", (gold_at, logged_in_user["user_id"])).rowcount
